...
const slideRight     = { hidden: { opacity: 0, x: 32 },   visible: { opacity: 1, x: 0,  transition: { duration: 0.55, ease: [0.4,0,0.2,1], delay: 0.05 } } };
const staggerContainer = { hidden: {}, visible: { transition: { staggerChildren: 0.1, delayChildren: 0.3 } } };
const featureItem    = { hidden: { opacity: 0, x: -12 },  visible: { opacity: 1, x: 0,  transition: { duration: 0.3, ease: 'easeOut' } } };
  ...
  return (
    <div className="auth-root">
      ...
      <div className="auth-brand">
        ...
        <motion.div className="auth-brand-content" variants={slideLeft} initial="hidden" animate="visible">
          <div className="auth-logo-wrap">
            <motion.div className="auth-logo-mark" variants={logoSpring} initial="hidden" animate="visible">💰</motion.div>
          </div>
          <motion.div className="auth-brand-name" variants={fadeUp} initial="hidden" animate="visible" style={{ transitionDelay: '0.15s' }}>
            Personal<br /><span className="auth-brand-name-accent">Budget</span>
          </motion.div>
          <motion.p className="auth-brand-tagline" variants={fadeUp} initial="hidden" animate="visible" transition={{ delay: 0.25 }}>
            Controle financeiro pessoal e familiar. Organize receitas, despesas, cartões e contas em um só lugar.
          </motion.p>
          <motion.ul className="auth-feature-list" variants={staggerContainer} initial="hidden" animate="visible">
            {[
              'Receitas e despesas por membro da família',
              'Cartões de crédito, contas bancárias e faturas',
              'Categorias personalizadas e relatórios visuais',
              'Sincronização em tempo real via API segura',
            ].map((f, i) => (
      ...
      <div className="auth-form-panel">
        <motion.div className="auth-form-card" variants={slideRight} initial="hidden" animate="visible">
          <AnimatePresence mode="wait">
            <motion.div key={tab + '-header'} className="auth-form-header"
          ...
          <div className="auth-tabs" role="tablist">
            {[['login','Entrar'],['register','Cadastrar']].map(([k, l]) => (
              <button key={k} role="tab" aria-selected={tab === k} className={`auth-tab${tab === k ? ' active' : ''}`} onClick={() => switchTab(k)} type="button">{l}</button>
            ))}
          </div>
          ...
          <form onSubmit={submit} noValidate>
            ...
            <AnimatePresence>
              {error && (
                <motion.div className="auth-error" role="alert" initial={{ opacity: 0, y: -8, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -4, scale: 0.97 }} transition={{ duration: 0.2 }}>
                  {error}
                </motion.div>
              )}
            </AnimatePresence>

            <motion.button type="submit" className="btn btn-primary auth-submit" disabled={loading} whileHover={{ scale: 1.015 }} whileTap={{ scale: 0.985 }} transition={{ type: 'spring', stiffness: 400, damping: 20 }}>
